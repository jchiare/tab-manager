...
async function addDedupedResultsToFrontend(numberOfDedupedTabs, dedupedTabUrls) {
    ...
    const dedupAmountElement = dedupNumbTemplate.content.firstElementChild.cloneNode(true);
    dedupAmountElement.querySelector('#dedup-amount').innerHTML =
        numberOfDedupedTabs > 1 ? `Deduplicated <strong>${numberOfDedupedTabs}</strong> tabs ` : `Deduplicated <strong>${numberOfDedupedTabs}</strong> tab `;

    dedupResultDiv.append(dedupAmountElement);

    for (const url of dedupedTabUrls) {
        // @ts-expect-error .. html elements are weird
        const urlElement = dedupedUrlsTemplate.content.firstElementChild.cloneNode(true);
...
async function deduplicateTabsByUrl() {
    const { numberOfDedupedTabs, dedupedTabUrls } = await findAndRemoveDuplicateTabs();
